...
import type { CaseStudy } from "@/content/portfolio-types"
import { unstable_noStore as noStore } from "next/cache"
import Image from "next/image"

type Props = {
  portfolio: ContentBundle["portfolio"]
  authorName: string
  studies: CaseStudy[]
}

export default function PortfolioPage({ portfolio, authorName, studies }: Props) {
  noStore()

  const { navItems, hero, about, domains, work, contact, footer } = portfolio

  return (
    <>
      <Navigation items={navItems} authorName={authorName} />
      ...
      <Section className="min-h-screen flex items-center pt-20" withGrid>
        <div className="w-full flex flex-col lg:flex-row items-center justify-between gap-10 lg:gap-14">
          ...
          <div className="max-w-4xl animate-fade-in">
            ...
            <p className="mt-8 ui-lead max-w-xl">{hero.tagline}</p>
      ...
      <Section id="domains" withDots>
        <SectionHeader label={domains.sectionLabel} title={domains.title} description={domains.description} />
      ...
      <Section id="case-studies" className="bg-card/30 overflow-visible">
        <SectionHeader label={work.sectionLabel} title={work.title} description={work.description} />
      ...
      <Section id="contact" withGrid className="py-14 md:py-16 lg:py-20">
        <div className="max-w-2xl mx-auto text-center">
          <div className="mb-8 md:mb-10 text-center">
            ...
            <h2 className="ui-h2-compact">{contact.title}</h2>
            <p className="mt-3 ui-sublead max-w-2xl mx-auto">{contact.description}</p>
          </div>
          ...
          <p className="mt-6 ui-mono-sm">{contact.email}</p>
        </div>
      </Section>

      <Footer authorName={authorName} socialLinks={footer.socialLinks} availabilityLabel={footer.availabilityLabel} />
    </>
  )
}
